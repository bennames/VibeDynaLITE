...
for k_penalty in [1e7, 1e6, 5e5, 1e5]:
    ...
    print(f"\n--- Testing k_penalty = {k_penalty:.1e} ---")
    for step in range(150):
        ...
        if step % 20 == 0 or step == 149 or np.sum(grid_b.failed) > 0:
            ...
            print(f"Step {step}: proj_pos={proj_pos_b[2]:.6f}, proj_vel={proj_vel_b[2]:.1f}, max_strain={max_strain:.4f}, failed_springs={n_failed}")
            if n_failed > 0 and proj_pos_b[2] > 0.01 and proj_vel_b[2] > 100.0:
