...
for step in range(200):
    (
        pos_b, vel_b, grid_b.failed, proj_pos_b, proj_vel_b,
        damp_diss, failure_diss, clamp_diss, t_sim, *_
    ) = fused_leapfrog_loop(
        ...
        dt, 1, 1, damp_diss, failure_diss, clamp_diss, t_sim, 1.0,
    ...
    if step % 10 == 0 or step >= 140:
        print(f"{step:4d} | {proj_ke:7.3f} | {grid_ke:7.3f} | {grid_se:7.3f} | {damp_diss:9.3f} | {failure_diss:12.3f} | {total_e:12.3f} | {max_strain:10.6f}")
