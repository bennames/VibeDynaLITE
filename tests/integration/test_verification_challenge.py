...
def run_simulation(
    ...
    while step < max_steps:
        ...
        if debug:
            print(
                f"Step {step:4d}: Total={total_energy:6.2f} | KE_nodes={ke_nodes:6.2f} | SE_springs={se_springs:6.2f} | KE_proj={ke_proj:6.2f} | Damp={damp_dissipated:6.2f} | Fail={failure_dissipated:6.2f} | Contact={contact_energy:6.2f} | Friction={friction_dissipated:6.2f}"
            )
...
def test_jonas_laval_curve_fit():
    ...
    for v_s in strike_vels:
        res = run_simulation(
            v_s, nx=31, ny=31, n_plies=1, k_penalty=2e5, mu_s=0.20, max_steps=600, grid=grid
        )
...
def test_energy_drift_verification():
    ...
    for mu in frictions:
        for k in stiffnesses:
            res = run_simulation(
                500.0, nx=31, ny=31, n_plies=13, k_penalty=k, mu_s=mu, max_steps=400, grid=grid
            )
            print(
                f"  Friction: {mu:.2f} | Stiffness: {k:.1e} | Energy Drift: {res['energy_drift_pct']:.3f}%"
            )
...
def test_physical_plausibility_failure_modes():
    ...
    res = run_simulation(
        450.0, nx=31, ny=31, n_plies=13, k_penalty=1e6, mu_s=0.20, max_steps=800, grid=grid
    )
    ...
    for ply in range(13):
        ...
        print(
            f"    Ply {ply:2d}: Total Failed = {total_failed:4d} | Shear/Diag = {failed_diag:4d} ({shear_ratio * 100:5.1f}%) | Tensile/Ortho = {failed_ortho:4d} ({strain_ratio * 100:5.1f}%)"
        )
