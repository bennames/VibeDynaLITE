...
def test_softened_force_decay():
    ...
    eff_k, step_fe = numba_compute_effective_k(
        positions, springs, stiffnesses, rest_lengths, failed,
        damage_onset_strain=0.1, failure_strain=0.2, grid_damage=grid_damage,
        spring_failed_step=spring_failed_step, current_step=100, fracture_energy_multiplier=1.0
    )
    ...
    forces_100, _ = numba_parallel_compute_spring_forces(
        positions, velocities, springs, eff_k, stiffnesses, rest_lengths, tension_only,
        node_spring_offsets, node_spring_ids, node_spring_signs, rayleigh_beta=0.0,
        failed=failed, spring_failed_step=spring_failed_step, current_step=100, erosion_softening_steps=10
    )
    ...
    forces_105, _ = numba_parallel_compute_spring_forces(
        positions, velocities, springs, eff_k, stiffnesses, rest_lengths, tension_only,
        node_spring_offsets, node_spring_ids, node_spring_signs, rayleigh_beta=0.0,
        failed=failed, spring_failed_step=spring_failed_step, current_step=105, erosion_softening_steps=10
    )
    ...
    forces_110, _ = numba_parallel_compute_spring_forces(
        positions, velocities, springs, eff_k, stiffnesses, rest_lengths, tension_only,
        node_spring_offsets, node_spring_ids, node_spring_signs, rayleigh_beta=0.0,
        failed=failed, spring_failed_step=spring_failed_step, current_step=110, erosion_softening_steps=10
    )
